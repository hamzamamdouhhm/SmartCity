...
RESOURCES = BASE / "backend" / "data" / "csv"
...
def parse_num(s):
    ...
    if s == "" or s == "0,00":
        return None
    # Handle percent signs and units
    s = s.replace("%", "").replace("Mbit/s", "").replace("m²", "").replace(" ", "")
    # Replace unicode minuses/dashes with ASCII minus
    s = s.replace("\u2212", "-").replace("\u2013", "-").replace("\u2014", "-")
    # German number format:
    #   1.234,56 -> 1234.56 (dot thousands, comma decimal)
    #   236.000  -> 236000  (dot thousands, no decimal)
    #   4,20     -> 4.20    (comma decimal)
    if "," in s and "." in s:
        # Both present: comma is decimal, dot is thousands
        s = s.replace(".", "").replace(",", ".")
    elif "," in s:
        # Only comma: ambiguous. If exactly 3 digits after comma treat as thousands, else decimal.
        parts = s.split(",")
        ...
        # Only dot: if exactly 3 digits after last dot and before is integer, treat as thousands
        parts = s.rsplit(".", 1)
        if len(parts) == 2 and len(parts[1]) == 3 and parts[1].isdigit() and parts[0].replace("-", "").isdigit():
            s = parts[0] + parts[1]
        ...
        "indicatorCatalogue": [
            {
                ...
                "csvKey": ind["csvKey"],
                "source": {
                    "Bandbreitenverfügbarkeit mindestens 100 Mbit/s": "Tabelle Abfrage1.csv / INKAR / BBSR",
                    "Mittelwert Festnetzempfang (Download) in Mbit/s": "Digitaler_Wandel_Benchmarking(Festnetz).csv",
                    "Mobilfunk O2": "Digitaler_Wandel_Benchmarking(Mobilfunk).csv",
                    "Mobilfunk Vodafone": "Digitaler_Wandel_Benchmarking(Mobilfunk).csv",
                    "Mobilfunk Telekom": "Digitaler_Wandel_Benchmarking(Mobilfunk).csv",
                    "OZG Reifegrad-Mittelwert": "Digitaler_Wandel_Benchmarking(Digitale Services).csv",
                    "OZG Prozentanteil ab Stufe 2": "Digitaler_Wandel_Benchmarking(Digitale Services).csv",
                    "Schulabdeckung": "Digitaler_Wandel_Benchmarking(Existenz Daseinsvorsorge).csv",
                    "Kitas-Abdeckung": "Digitaler_Wandel_Benchmarking(Existenz Daseinsvorsorge).csv",
                    "Gewerbegebiet pro 1.000 EW": "Digitaler_Wandel_Benchmarking(Existenz Daseinsvorsorge).csv",
                }.get(ind["csvKey"], "Tabelle Abfrage1.csv / INKAR / BBSR"),
                "description": ind["description"],
